...
class SafeCheckpointCallback(transformers.TrainerCallback):
    def on_save(self, args, state, control, **kwargs):
        """Saves only the model and tokenizer, ignoring optimizer states."""
        ...
        if trainer.args.should_save:
            ...
            print(f"✅ Model checkpoint saved at: {checkpoint_dir} (without optimizer states)")

        # ✅ Prevent Trainer from saving optimizer states
        control.should_save = False  # Stop default save behavior
...
class CleanupCallback(transformers.TrainerCallback):
    def on_save(self, args, state, control, **kwargs):
        ...
        if not checkpoint_dirs:
            print("⚠️ No DeepSpeed checkpoints found for cleanup.")
            return control
        ...
        optimizer_files = [
            ...
            *glob.glob(os.path.join(latest_checkpoint, "*_model_states.pt")),
        ]
        ...
        return control  # Ensure training proceeds normally
...
# Function to replace special tokens
def clean_text(text):
...
class InstructionDataset_S(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, split='train', task='ag_news'):
        ...
        logging.warning(f"Loading {split} data...")
        ...
        for u in task_splits:
            ...
            logging.warning(f"Formatting {split} inputs...")
            ...
            deals = [clean_text(str(u)) for u in data['tgt']]
            targets = [f"{example}{tokenizer.eos_token}" for example in deals]

            logging.warning(f"Tokenizing {split} inputs... This may take some time...")
            data_dict = preprocess(sources, targets, tokenizer)

            train_idx, val_idx = train_val_split(data_dict, train_percent=0.985)
            ...
            input_ids, labels = get_data_by_idx(data_dict, selected_idx)

            self.input_ids.extend(input_ids)
            self.labels.extend(labels)

        print(f'The number of {split} data samples: {len(self.input_ids)}')
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_path,t) -> Dict:
    ...
    train_dataset = InstructionDataset_S(tokenizer=tokenizer, data_path=data_path, task=t, split='train')
    eval_dataset = InstructionDataset_S(tokenizer=tokenizer, data_path=data_path, task=t, split='val')  # Add validation dataset
...
def train():
    ...
    for i in range(len(task)):
        ...
        if i == 0:
            ...
            trainer = Trainer(model=model, tokenizer=tokenizer, args=t_args, **data_module, callbacks=[CleanupCallback(), SafeCheckpointCallback()])
            trainer.train()
            trainer.save_state()
